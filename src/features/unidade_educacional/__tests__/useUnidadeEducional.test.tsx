...
const mockBuscarUnidadeEducaionalPorUuid = vi.mocked(buscarUnidadeEducacionalPorUuid);

const mockAtualizarUnidadeEducacional = vi.mocked(atualizarUnidadeEducacional);
...
describe("useUnidadeEducacional", () => {
  ...
  describe("useUnidadeEducacional", () => {
    it("deve chamar o serviço com o UUID informado", async () => {
      mockBuscarUnidadeEducaionalPorUuid.mockResolvedValue(UNIDADE_EDUCACIONAL);
      ...
      await waitFor(() => {
        expect(mockBuscarUnidadeEducaionalPorUuid).toHaveBeenCalledWith(UUID);
      });
    });
    ...
    it("deve retornar os dados após sucesso", async () => {
      ...
      const { result } = renderHook(() => useUnidadeEducacional(UUID), {
        wrapper: criarWrapper(queryClient),
      });

      await waitFor(() => {
        expect(result.current.data).toEqual(UNIDADE_EDUCACIONAL);
      });
    ...
    it("deve estar pendente inicialmente", () => {
      mockBuscarUnidadeEducaionalPorUuid.mockReturnValue(new Promise(() => {}));
    ...
    it("não deve chamar o serviço quando o UUID estiver vazio", () => {
      const { result } = renderHook(() => useUnidadeEducacional(""), {
        wrapper: criarWrapper(queryClient),
      });

      expect(mockBuscarUnidadeEducaionalPorUuid).not.toHaveBeenCalled();
    ...
    it("deve usar o UUID informado na queryKey", async () => {
      ...
      await waitFor(() => {
        expect(mockBuscarUnidadeEducaionalPorUuid).toHaveBeenCalledTimes(1);
      });

      const [query] = queryClient.getQueryCache().findAll();

      expect(query.queryKey).toEqual(["unidade", UUID]);
    });
    ...
    it("deve atualizar a consulta quando o UUID mudar", async () => {
      ...
      const { rerender } = renderHook(({ uuid }: { uuid: string }) => useUnidadeEducacional(uuid), {
        initialProps: {
          uuid: UUID,
        },
        wrapper: criarWrapper(queryClient),
      ...
      await waitFor(() => {
        expect(mockBuscarUnidadeEducaionalPorUuid).toHaveBeenCalledWith(novoUuid);
      });

      expect(mockBuscarUnidadeEducaionalPorUuid).toHaveBeenCalledTimes(2);
    });

    it("deve retornar erro quando o serviço falhar", async () => {
      const erro = new Error("Erro ao buscar unidade educacional");

      mockBuscarUnidadeEducaionalPorUuid.mockRejectedValue(erro);
  ...
  describe("useAtualizarUnidadeEducacional", () => {
    const PAYLOAD = {
    ...
    it("deve chamar o serviço com UUID e payload informados", async () => {
      mockAtualizarUnidadeEducacional.mockResolvedValue({
        success: true,
      });

      const { result } = renderHook(() => useAtualizarUnidadeEducacional(UUID), {
        wrapper: criarWrapper(queryClient),
      });

      await result.current.mutateAsync(PAYLOAD);

      expect(mockAtualizarUnidadeEducacional).toHaveBeenCalledWith(UUID, PAYLOAD);
    });

    it("deve invalidar as consultas após atualização com sucesso", async () => {
      mockAtualizarUnidadeEducacional.mockResolvedValue({
        success: true,
      });
    ...
    it("não deve invalidar as consultas quando a atualização retornar erro da API", async () => {
      mockAtualizarUnidadeEducacional.mockResolvedValue({
        success: false,
        error: "api-error",
        title: "Não foi possível atualizar",
        message: "Erro ao atualizar unidade educacional.",
        status: 400,
      });
